...
fn pair_to_rust(pair: Pair<Rule>) -> String {
    let mut s = String::new();
    let rule = pair.as_rule();

    // 子规则数量不固定, 类型也不一样, 用 for subpair in pair.into_inner() { match subpair.as_rule() { 解析
    match rule {
            ...
            for subpair in pair.into_inner() {
                match subpair.as_rule() {
                    | Rule::identifier | Rule::identifier_atomic => {
                        identifier.push_str(&pair_to_rust(subpair));
                        identifier.push_str("_");
                    },
                    | Rule::expression => {
                        parameters.push_str(&pair_to_rust(subpair));
                        parameters.push_str(",");
                    },
                    | _ => {},
                }
        ...
        | Rule::array_some => {
            s.push_str("vec![");
            for subpair in pair.into_inner() {
                s.push_str(&pair_to_rust(subpair));
                s.push_str(",");
            }
            s.push_str("]");
            return s;
        ...
        | Rule::break_statement => {
            ...
            for subpair in pair.into_inner() {
                match subpair.as_rule() {
                    | Rule::loop_mark => {
                        mark.push_str(&pair_to_rust(subpair));
                        mark.pop();
                    },
                    | Rule::expression => {
                        expr.push_str(&pair_to_rust(subpair));
                    },
                    | _ => {},
                }
            }
        ...
        | Rule::continue_statement => {
            ...
            for subpair in pair.into_inner() {
                s.push_str("'");
                s.push_str(&pair_to_rust(subpair.into_inner().next().unwrap()));
            }
            s.push_str(";");
            return s;
        },
        ...
        | Rule::array_index_expr => {
            let mut inner_rules = pair.into_inner();
            s = format!(
                "{}[{}]",
                pair_to_rust(inner_rules.next().unwrap()),
                pair_to_rust(inner_rules.next().unwrap()),
            );
            return s;
        },
        ...
        | Rule::array_type => {
            let mut inner_rules = pair.into_inner();
            s = format!("Vec<{}>", pair_to_rust(inner_rules.next().unwrap()),);
            return s;
        },
        | Rule::assignment => {
            let mut inner_rules = pair.into_inner();
            s = format!("= {}", pair_to_rust(inner_rules.next().unwrap()));
            return s;
        },
        | Rule::binary_expr => {
            let mut inner_rules = pair.into_inner();
        ...
        | Rule::else_branch => {
            let mut inner_rules = pair.into_inner();
            s = format!("else {}", pair_to_rust(inner_rules.next().unwrap()));
            return s;
        },
        | Rule::else_if_branch => {
            let mut inner_rules = pair.into_inner();
        ...
        | Rule::loop_times_block => {
            ...
            if inner_rule.as_rule() == Rule::loop_mark {
                loop_mark = pair_to_rust(inner_rule);
            ...
            if inner_rule.as_rule() == Rule::expression {
                ...
                s.push_str(" while ( i < ( ");
                s.push_str(&pair_to_rust(inner_rule));
                s.push_str(")) { i = i + 1;");
                ...
                s = format!("{} loop {}", &loop_mark, pair_to_rust(inner_rule));
            }
            return s;
        },
        | Rule::loop_for_block => {
        ...
        | Rule::loop_repeat_block => {
            ...
            if inner_rule.as_rule() == Rule::loop_mark {
                ...
                s = format!("loop {}", pair_to_rust(inner_rule));
            }
            return s;
        },
        ...
        | Rule::match_else_branch => {
            let mut inner_rules = pair.into_inner();
            s = format!("_ => {}", pair_to_rust(inner_rules.next().unwrap()));
            return s;
        },
        | Rule::mut_assignment => {
            let mut inner_rules = pair.into_inner();
        ...
        | Rule::triple_quote_string => {
            s.push_str("\"");
            s.push_str(&pair_to_rust(pair.into_inner().next().unwrap()));
            s.pop();
            s.push_str("\"");
            return s;
        },
        ...
        | Rule::string_literal
        | Rule::type_name => {
            return pair_to_rust(pair.into_inner().next().unwrap());
        },
